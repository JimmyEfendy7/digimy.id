...
export default function CustomerLoginPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/account/login`, {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/account/login/verify`,
        {
          phone_number: phoneNumber,
          otp_code: otpValue,
        }
      ...
      if (response.data.success) {
        setSubmitStatus('success');
        setStatusMessage('Login berhasil! Mengarahkan ke halaman utama...');
            ...
            <div className="p-6 md:p-8">
              ...
              <form onSubmit={otpSent ? handleVerifyOtp : handleSendOtp} className="space-y-6">
                  ...
                  >
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors text-center text-xl tracking-widest ${
                ...
                <button
                  ...
                  className="w-full bg-green-600 text-white py-3 px-4 rounded-lg hover:bg-green-700 focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
                    <Loader2 className="w-5 h-5 animate-spin" />
                  ) : (
                    <>
                      <span>{otpSent ? 'Verifikasi & Masuk' : 'Kirim Kode OTP'}</span>
              ...
              <div className="mt-6 text-center">
                <p className="text-sm text-gray-600">
                  Belum punya akun?{' '}
                  <Link href="/account/register" className="text-green-600 hover:text-green-700 font-medium">
                    Daftar di sini
                  </Link>
                </p>
              </div>
